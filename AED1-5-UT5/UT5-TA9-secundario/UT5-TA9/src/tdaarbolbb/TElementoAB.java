...
public class TElementoAB<T> implements IElementoAB<T> {
    ...
    protected T datos;
    protected Integer frecuenciaExito;
    protected Integer frecuenciaNoExitoIzquieda;
    protected Integer frecuenciaNoExitoDerecha;
    ...
    public Integer getFrecExito() {
        return frecuenciaExito;
    }

    public Integer getFrecNoExitoIzquierda() {
        return frecuenciaNoExitoIzquieda;
    }

    public Integer getFrecNoExitoDerecha() {
        return frecuenciaNoExitoDerecha;
    }

    public void incrementarFrecuenciaExito() {
        frecuenciaExito += 1;
    }

    public void incrementarFrecuenciaNoExitoIzquierda() {
        frecuenciaNoExitoIzquieda += 1;
    }

    public void incrementarFrecuenciaNoExitoDerecha() {
        frecuenciaNoExitoDerecha += 1;
    }
    ...
    @Override
    public int calcularCosto(int[] FrecExito, int[] FrecNoExito, int[] contador, int nivel) {
        int costo = 0;
        if (hijoIzq != null) {
            costo = hijoIzq.calcularCosto(FrecExito, FrecNoExito, contador, nivel + 1);
        } else {
            costo = (nivel + 1) * FrecNoExito[contador[0]];
            contador[0]++;
        }
        costo += nivel * FrecExito[contador[0]];

        if (hijoDer != null) {
            costo += hijoDer.calcularCosto(FrecExito, FrecNoExito, contador, nivel + 1);
        } else {
            costo += (nivel + 1) * FrecNoExito[contador[0]];
            contador[0]++;
        }
    ...
    public void cuentaFrec(Comparable unaClave) {
        if (unaClave.compareTo(this.etiqueta) == 0) {
            incrementarFrecuenciaExito();
        } else if (unaClave.compareTo(this.etiqueta) < 0) {
            if (hijoIzq != null) {
                hijoIzq.cuentaFrec(unaClave);
            } else {
                incrementarFrecuenciaNoExitoIzquierda();
            }
        } else {
            if (hijoDer != null) {
                hijoDer.cuentaFrec(unaClave);
            } else {
                incrementarFrecuenciaNoExitoDerecha();
            }
        }
    }
    ...
    public void completarVectores(String[] claves, int[] frecExito, int[] frecNoExito) {
        ...
        if (hijoIzq != null) {
            hijoIzq.completarVectores(claves, frecExito, frecNoExito);
        }

        for (int i = 1; i < claves.length; i++) {
            if (claves[i] == etiqueta) {
                frecExito[i] = this.frecuenciaExito;
                frecNoExito[i] = this.frecuenciaNoExitoDerecha + this.frecuenciaNoExitoIzquieda;
            }
        }

        if (hijoDer != null) {
            hijoDer.completarVectores(claves, frecExito, frecNoExito);
        }
